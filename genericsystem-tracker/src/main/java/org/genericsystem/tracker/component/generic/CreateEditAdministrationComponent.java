...
public class CreateEditAdministrationComponent extends AbstractGenericCollectableChildrenComponent {
	private String title;

	private RowAdminComponent instanceRow;

	public CreateEditAdministrationComponent(AbstractComponent parent, Generic generic) {
		super(parent, generic);
		title = generic.isStructural() ? "add" : "update";
		instanceRow = new RowAdminComponent(this, generic);
	}
	...
	public <T extends Generic> boolean isSelected(T candidate) {
		...
		return clazz != null && (org.genericsystem.security.structure.Attributes.class.equals(clazz) || org.genericsystem.security.structure.Relations.class.equals(clazz));

	}

	@Override
	public <T extends AbstractComponent, U extends Generic> T buildComponent(U generic) {
		return (T) new RowAdminComponent(this, generic);
	}

	public String execute() {
		List<RowAdminComponent> rows = this.<RowAdminComponent> getChildren();
		...
		for (int i = 1; i < rows.size(); i++) {
			...
			for (AbstractComponent component : rows.get(i).getChildren()) {
				if (component instanceof OutputTextComponent)
					...
					if (org.genericsystem.security.structure.Attributes.Password.class.equals(attribute.getClass()))
						getGeneric().setValue((Attribute) attribute, MD5Hashing.hashPassword((((InputTextComponent) component).getNewValue()).toString()));
					else
