...
public class GenericImpl implements Generic, Type, Link, Relation, Value, Attribute, Property, Tree, Node {
	...
	final Generic initPrimary(Serializable value, int metaLevel, Generic primaryAncestor) {
		return restore(value, metaLevel, null, Long.MAX_VALUE, 0L, Long.MAX_VALUE, new Generic[] { primaryAncestor }, new Generic[] { primaryAncestor }, Statics.EMPTY_GENERIC_ARRAY);
	}

	final GenericImpl initialize(Generic[] interfaces, Generic[] directSupers, Generic[] components) {
		return restore(interfaces[0].getValue(), interfaces[0].getMetaLevel(), null, Long.MAX_VALUE, 0L, Long.MAX_VALUE, interfaces, directSupers, components);
	}

	final GenericImpl restore(Serializable value, int metaLevel, Long designTs, long birthTs, long lastReadTs, long deathTs, Generic[] interfaces, Generic[] directSupers, Generic[] components) {
		this.value = value;
	...
	public <T extends Generic> T getImplicit() {
		return (T) (isPrimary() ? this : interfaces[0]);
	}

	@Override
	public EngineImpl getEngine() {
		return (EngineImpl) interfaces[0].getEngine();
	}
	...
	public int getMetaLevel() {
		return isPrimary() ? metaLevel : getImplicit().getMetaLevel();
	}
	...
	public <S extends Serializable> S getValue() {
		return (S) (isPrimary() ? value : getImplicit().getValue());
	}
	...
	private <T extends Link> T setToManyLink(Cache cache, Link property, Serializable value, int basePos, Generic... targets) {
		if (((Type) property).isPropertyConstraintEnabled(cache)) {
			...
			if (link == null)
				return addLink(cache, property, value, basePos, targets);
			if (!this.equals(link.getBaseComponent()))
				return addLink(cache, link, value, basePos, targets);
			if (Objects.equals(value, link.getValue()))
				return link;
			return update(cache, link, value);
	...
	// throw new CacheContextAwareException("Cache context is required for doing this !");
	...
	public <T extends Generic> T newInstance(Cache cache, Serializable value, Generic... components) {
		return ((CacheImpl) cache).add(this, value, getMetaLevel() + 1, Statics.truncate(0, this.interfaces), components);
	}

	@Override
	public <T extends Type> T newSubType(Cache cache, Serializable value, Generic... components) {
		return ((CacheImpl) cache).add(this, value, getMetaLevel(), Statics.truncate(0, interfaces), components);
	}
	...
	private boolean safeIsEnabled(Context context, Property property) {
		Iterator<Generic> iterator = new AbstractMagicIterator(context, property) {
			@Override
			protected boolean isSelected(Generic candidate) {
				return (candidate.getMetaLevel() <= SystemGeneric.CONCRETE) && candidate.isAttributeOf(GenericImpl.this, Statics.BASE_POSITION);
			}
	...
	public boolean inheritsFrom(Generic generic) {
		if (generic == null)
			return false;
		boolean superOf = ((GenericImpl) generic).new InheritanceCalculator().isSuperOf(this);
		boolean superOf2 = ((GenericImpl) generic).isSuperOf(this);
		assert superOf == superOf2 : "" + this.info() + generic + " : " + superOf;
		return superOf2;
	}
	...
	public boolean isSuperOf(Generic generic) {
		...
		if (((GenericImpl) generic).isPrimary())
			return isSuperOf(((GenericImpl) generic).interfaces[0]);

		return isSuperOf(isPrimary() ? new Generic[] { this } : this.interfaces, this.components, ((GenericImpl) generic).interfaces, ((GenericImpl) generic).components);
	}
	...
	public static boolean isSuperOf(Generic[] interfaces, Generic[] components, final Generic[] subInterfaces, Generic[] subComponents) {
		...
		if (interfaces.length == subInterfaces.length && components.length == subComponents.length) {
			...
			for (int i = 0; i < subComponents.length; i++) {
				if (subComponents[i] != null/* && components[i] != null */)
					if (!Arrays.equals(interfaces, ((GenericImpl) components[i]).interfaces) || !Arrays.equals(subInterfaces, ((GenericImpl) subComponents[i]).interfaces) || !Arrays.equals(components, ((GenericImpl) components[i]).components)
	...
	@Override
	public <T extends Generic> Snapshot<T> getInterfaces() {
		return new AbstractSnapshot<T>() {
	...
	private <T extends Generic> Iterator<T> interfacesIterator() {
		return new ArrayIterator<T>((T[]) interfaces);
	}
	...
	public String info() {
		...
		for (Generic interface_ : interfaces)
	...
	public String toString() {
		...
		return Arrays.toString(interfaces) + "/" + toString(components);
	}
	...
	public boolean isPrimary() {
		return components.length == 0 && interfaces.length == 1;
	}
