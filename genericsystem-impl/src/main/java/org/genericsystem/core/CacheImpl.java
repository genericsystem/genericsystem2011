...
public class CacheImpl extends AbstractContext implements Cache {
	...
	private Generic getNewPrimary(Generic oldSubPrimary, Generic oldPrimary, Generic newPrimary) {
		...
		if (oldSubPrimary.equals(oldPrimary))
			return newPrimary;
		return bindPrimaryByValue(getNewPrimary(((GenericImpl) oldSubPrimary).supers[0], oldPrimary, newPrimary), oldSubPrimary.getValue(), oldSubPrimary.getMetaLevel(), oldSubPrimary.isAutomatic());
	}
	...
	public <T extends Tree> T newTree(Serializable value, int dim) {
		return this.<T> bind(bindPrimaryByValue(getEngine(), value, SystemGeneric.STRUCTURAL, true), Statics.EMPTY_GENERIC_ARRAY, new Generic[dim], false, null, false).<T> disableInheritance(this);
	}
